...
if(!empty($_GET['id']))
{
	...
	{
		$req=$db->prepare("DELETE FROM rendezVous WHERE pkArtif=?");
		$req->execute(array($_GET['id']));
		header("Location: index.php?page=general/message&type=confirm&msg=Le rendez-vous a bien été supprimé.&retour=rdv/list");
	}
