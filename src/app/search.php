<?php
		...
		{
		$id = $_GET['id'];
		$req=$db->prepare("select * from contact c, posteContact p where c.identifiant=? AND c.identifiant=p.salarie");
		...
		$display.="Poste : ".$result['poste']."<BR>" ;
		$display.="Entreprise : ".$result['entreprise']."<BR><BR><BR><BR>" ;
